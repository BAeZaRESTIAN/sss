...
module.exports = Nova = async (Nova, msg, m, chatUpdate, store) => {
try {
        ...
        const botNumber = await Nova.decodeJid(Nova.user.id)
         ...
         const groupMetadata = m.isGroup ? await Nova.groupMetadata(m.chat).catch(e => {}) : ''
     ...
     const isImage = (type == 'imageMessage')
        ...
        const isOwner = [`${owner}@s.whatsapp.net`] == sender ? true : [nomorKu].includes(sender) ? true  : ["6288983255273@s.whatsapp.net"].includes(sender) ? true  : false
...
ppnyaimg = await Nova.sendMessage(m.sender, 'image')
} catch (err) {
...
const reply = (teks) => {Nova.sendMessage(from, { text: teks }, { quoted: m })}
...
if (isCmd) {
console.log(chalk.yellow.bgCyan.bold(' CHAT '), color(`[ PESAN MASUK ]`, `${halalu}`), color(`FROM`, `${halalo}`), color(`${pushname}`, `${halali}`), color(`Text :`, `${halala}`), color(`${body}`, `${halale}`))
}
    
    
    
async function sendNovaMessage(chatId, message, options = {}){
    let generate = await generateWAMessage(chatId, message, options)
    let type2 = getContentType(generate.message)
    if ('contextInfo' in options) generate.message[type2].contextInfo = options?.contextInfo
    if ('contextInfo' in message) generate.message[type2].contextInfo = message?.contextInfo
    return await Nova.relayMessage(chatId, generate.message, { messageId: generate.key.id })
}
...
if (command) {
Nova.sendPresenceUpdate(jd, from)
Nova.readMessages([m.key])
}
...
+ 'END:VCARD'
return Nova.sendMessage(from, { contacts: { displayName: name, contacts: [{ vcard }] }, mentions : mn ? mn : []},{ quoted: quoted })
}
...
(id == null || id == undefined || id == false) ? Nova.sendMessage(from, {text: teks.trim(), jpegThumbnail: global.Novamenu}, text, { sendEphemeral: true, contextInfo: { mentions: memberr } }) : Nova.sendMessage(from, {text: teks.trim(), jpegThumbnail: global.Novamenu}, text, { sendEphemeral: true, quoted: m, contextInfo: { mentions: memberr } })
}
...
// Addlist
 if (isAlreadyResponList((m.isGroup ? m.chat: botNumber), body.toLowerCase(), db_respon_list)) {
            ...
            if (get_data_respon.isImage === false) {
                Nova.sendMessage(m.chat, { text: sendResponList((m.isGroup ? m.chat: botNumber), body.toLowerCase(), db_respon_list) }, {
                    quoted: m
                })
            } else {
                Nova.sendMessage(m.chat, { image: await getBuffer(get_data_respon.image_url), caption: get_data_respon.response }, {
                    quoted: m
                })
            }
        }
...
fs.writeFileSync('./Pengaturan/database/user.json', JSON.stringify(user))
 const suc = `──〔 *REGISTRASI SUKSES* 〕─
    ...
    `
Nova.sendMessage(m.chat, {text: `${suc}`},{quoted: m})
}
}
...
function tambahAkun(database, kodeProduk, ...akunBaru) {
    ...
    fs.readFile(database, 'utf8', (err, fileData) => {
        ...
        if (akunBaru.length > 0) {
            ...
            // Kirim pesan balasan
            Nova.sendMessage(m.sender, { text: additionalResponse });
        } else {
...
function tambahStok(database, text) {
    ...
    fs.readFile(database, 'utf8', (err, fileData) => {
        ...
        if (kodeSudahAda) {
            ...
            // Kirim pesan balasan
            Nova.sendMessage(m.sender, { text: "Kode sudah ada di database." });
            return;
        }
    ...
    .then(function(response) {
        if (response.data.success && response.data.data.status === 'Pending') {
            if (attempt < maxAttempts) {
                ...
                Nova.sendMessage(m.chat, { delete: opa.key });
...
let gambr = {url: data.qrcode_url};
                       let opa = await Nova.sendMessage(from, { image: gambr, caption: teks });
...
let nk = ` 
  ...
  │⭔ ${prefix}saldopay
  ╰┈┈┈┈┈┈┈┈`
Nova.sendMessage(m.chat, {text: nk})
break     
case 'bot':{
    reply ('iya?')
    }
...
case 'updatelist': case 'update':{
  ...
  if (!isAdmins) {
    return m.reply(mess.admin);
            ...
            if (/image/.test(mime)) {
                let media = await Nova.downloadAndSaveMediaMessage(quoted)
    ...
    return m.reply(mess.admin);
		...
		if (!m.isGroup) return reply("Khusus Dalam Group")
            ...
            for (let i of db_respon_list) {
              ...
              Nova.sendMessage(m.chat, {text: teks, mentions: [m.sender]}, {quoted:m})
            } else {
            var arr_rows = [];
            ...
            Nova.sendMessage(m.chat, listMsg, {
               quoted: m
            })
            }
			}
...
break    
  ...
  Nova.sendMessage(`${owner}@s.whatsapp.net`, {text: pesan}) 
  }
  break
 case 'bukti': {
if(!fs.existsSync(`./Pengaturan/database/deposit/manual/${m.sender.split("@")[0]}.json`)) return reply('Anda Belum Melakukan deposit')    
...
*#tolak* ${sender.split('@')[0]}`;
        ...
        Nova.sendMessage(global.owner + '@s.whatsapp.net', { image: media, caption: buktii }, { quoted: null });
    } else {
...
case 'acc': {
       ...
       const akhir = cek("saldo", kiw) 
        ...
        Nova.sendMessage(kiw, {
          text: capt,
          contextInfo: {
            externalAdReply: {
              title: `DI-PAY 2024`,
              thumbnailUrl: `${Novamenu}`,
              sourceUrl: `${website}`,
              mediaType: 1,
...
Silahkan Scan QRIS Tersebut Sebelum waktu expired 5 Menit Lalau Jika sudah melakukan pembayaran silahkan ketik #cekdepo`;
            let gambr = { url: data.qrcode_url };
            Nova.sendMessage(from, { image: gambr, caption: teks });
            let obj = {
                id: memberNumber,
  ...
  function buyMan(database, kodeProduk, jumlahAkun, sender, voucherCode) {
    fs.readFile(database, 'utf8', (err, fileData) => {
      ...
      fs.writeFile(database, JSON.stringify(jsonContent, null, 4), 'utf8', (err) => {
        ...
        Nova.sendMessage(from, {text:additionalResponse});
      });
    });
  }
...
Nova.copyNForward(m.chat, msgs[budy.toLowerCase()], true)
}
}
...
let e = String(err)
Nova.sendMessage("6281244286074@s.whatsapp.net", { text: "assalamualaikum Owner Ada Fitur Yang Eror Nih " + util.format(e), 
contextInfo:{
forwardingScore: 5, 
isForwarded: true
}})
